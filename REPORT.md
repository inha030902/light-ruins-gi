...
처음 플레이하는 사람이 다음 행동을 알 수 있도록 단계별 안내 프롬프트를 추가했다. 게임 시작 시에는 흰 별 기둥을 찾으라고 안내하고, GI 기동 후에는 유물 색에 맞게 받침대로 옮기라고 안내한다. Stage 1과 Stage 2에서 유물 배치가 끝나면 열린 문으로 이동하라는 메시지를 띄우고, 다리 구간에서는 `Space`로 점프하라고 안내한다.

![단계별 안내 프롬프트](./report-images/17-guide-prompt.png)

### 3.9 엔딩

Stage 3에서 세 유물을 모두 올바르게 배치하면 유적 복원이 완료된 것으로 처리된다. 이때 엔딩 오버레이가 바로 표시되고, 하단 안내 문구를 통해 마지막 유물 배치가 완료되었음을 확인할 수 있다.
